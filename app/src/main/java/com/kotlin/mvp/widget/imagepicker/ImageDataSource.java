package com.kotlin.mvp.widget.imagepicker;
...
public class ImageDataSource implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if ((activity.getLifecycle().getCurrentState() == Lifecycle.State.STARTED || activity.getLifecycle().getCurrentState() == Lifecycle.State.RESUMED) && imageFolders.size() > 0) {
            return;
        }
        imageFolders.clear();
        //查询数据
        if (data != null) {
            ...
            while (data.moveToNext()) {
                ...
                if (!(imageMimeType.toLowerCase().endsWith("jpg")||imageMimeType.toLowerCase().endsWith("jpeg")||imageMimeType.toLowerCase().endsWith("png"))) {
                    continue;  //只取jpg,png,jpeg格式图片
                }
